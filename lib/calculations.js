/* ═══════════════════════════════════════
   ...
   Importa helpers de utils.js internamente
   ═══════════════════════════════════════ */
...
export function getFeriado(dataStr, feriadosPersonalizados) {
...
export function isSabado(dataStr) {
...
export function calcularMinutosNoturnos(batidas) {
  if (!batidas || batidas.length < 2) return 0
  const validas   = batidas.filter(b => b && validarFormatoHora(b))
  const ordenadas = [...validas].sort((a, b) => horaParaMinutos(a) - horaParaMinutos(b))
...
export function analisarIntrajornada(batidas, totalMinutosTrabalhados) {
...
export function calcularValorHora(salario, horasMensais) {
  if (!salario || !horasMensais || horasMensais <= 0) return 0
  return salario / horasMensais
}

export function calcularValorHoraExtra(valorHora, percentual) {
  return valorHora * (1 + (percentual || 50) / 100)
}

/* ─── RESUMO COMPLETO CLT ─── */

export function calcularResumo(registro, config) {
  ...
  // Dia com batida sem par: expediente em andamento — não calcular saldo definitivo
  const incompleto = batidas.filter(b => b).length % 2 !== 0

  return {
    horasTrabalhadas,
    horasExtras:          incompleto ? 0 : horasExtrasMin,
    horasNormais:         incompleto ? 0 : horasNormaisMin,
    bancoHoras:           incompleto ? 0 : bancoHoras,
    extrasParaPagamento:  incompleto ? 0 : extrasParaPagamento,

    valorHora,
    valorHoraExtra,
    valorHorasNormais:    incompleto ? 0 : valorHorasNormais,
    valorExtras:          incompleto ? 0 : valorExtras,
    valorNoturno:         incompleto ? 0 : valorNoturno,
    valorSabado:          incompleto ? 0 : valorSabado,
    valorDomingo:         incompleto ? 0 : valorDomingo,
    totalDia:             incompleto ? 0 : totalDia,

    isDomingo:   ehDomingo,
    isSabado:    ehSabado,
    isFeriado:   ehFeriado,
    nomeFeriado: feriadoNome,
    minutosNoturnos: incompleto ? 0 : minutosNoturnos,

    alertaIntrajornada:  incompleto ? null  : alertaIntrajornada,
    alertaHoraExtraCLT:  incompleto ? false : alertaHoraExtraCLT,
...
export function calcularResumoMensal(registros, config, mes, ano) {
...
export function calcularResumoAnual(registros, config, ano) {
...
export function obterUltimosMeses(registros, config, n = 6) {
  const result = []
  const hoje   = new Date()
  for (let i = n - 1; i >= 0; i--) {
